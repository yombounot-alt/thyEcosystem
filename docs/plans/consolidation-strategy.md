...
#### Statut du portage (2026-09-23) — ✅ backend et app mobile portés et vérifiés

Base du portage : l'arbre de travail de thyBusiness au 23/09 (dont le WIP RBAC non commité) ; thyBusiness n'a **pas** été modifié.

- **Code** : `backend/src/modules/business/` (categories, products [+photos], inventory, customers, credits, sales, expenses, dashboard, payments [+moyens, logos, preuves]). Les modules `auth`/`members` de thyBusiness sont remplacés par le kernel (`modules/auth`, `modules/businesses`).
- **Schéma** : `backend/migrations/0004_biz_init.sql` (schéma `biz`, 12 tables, RLS forcée sur toutes) + `0003_core_rbac_business_permissions.sql` (permissions Business et surcharges par membre). `backend/prisma/schema.prisma` ne décrit que `biz` ; **Prisma n'est qu'un client de requêtes** — jamais `prisma migrate` (le test `schema-drift` détecte tout écart avec les migrations SQL).
- **Accès aux données** : `BizPrisma.run(businessId, fn)` ouvre une transaction et pose `app.business_id` ; toute requête `biz.*` passe par là (RLS fail-closed).
- **Correspondance des droits** : les 4 rôles de thyBusiness sont couverts par les rôles du kernel ; chaque route déclare une permission (`catalog:view`, `sales:create`, `payments:verify`, `payment_methods:manage`, `reports:view`, `finance:view_profit`…). Sans `finance:view_profit`, le tableau de bord renvoie `cogs`/`profit` à `null` (le chiffre d'affaires reste visible).
- **Écarts assumés par rapport à thyBusiness** : (1) index uniques partiels SKU/code-barres par entreprise ajoutés ; (2) enveloppe d'erreur du kernel `{ error: { code, message, requestId } }` au lieu de `{ message }` ; (3) inscription/connexion par OTP unique (plus de mot de passe) ; (4) identifiants invalides (UUID mal formé) → 404 ; (5) montants conservés en `Decimal(14,2)`.
- **Piège découvert** : sous RLS, PostgreSQL retire le détail « Key (…)=(…) » des violations d'unicité, donc Prisma renvoie `meta.target` absent ou `null` — `common/db-errors.ts` (`isUniqueViolation`) traite « cible inconnue » comme « peut-être » et le code revérifie par une lecture (vente retrouvée par `clientRequestId`).
- **Tests backend** : les 6 suites e2e de thyBusiness portées sur vitest (`backend/test/business/`, 118 tests, dont paiements manuels et idempotence concurrente) + suites du kernel (`backend/test/kernel/` : auth/OTP/refresh, RBAC/invitations/surcharges, `/me`, RLS au niveau SQL) + test anti-dérive Prisma↔SQL + tests unitaires — **217 tests verts** (169 e2e + 48 unitaires).
- **App mobile** : `mobile/app` (paquet Dart `thy_app`, code de `apps/mobile` de thyBusiness) — analyse sans alerte, **211 tests Flutter verts** + **6 tests de contrat exécutés contre le vrai backend** (`test/live_api_test.dart` : produits, photos, ventes hors-ligne, crédit, paiements, tableau de bord). Ces tests créent leur propre compte : lancer le backend avec `DEV_FIXED_OTP=123456` (jamais en production, refusé par la config) puis `flutter test test/live_api_test.dart --dart-define=THY_API_BASE_URL=http://localhost:3300/api/v1`.
- **Écarts côté app** : (1) un seul parcours téléphone → code SMS (plus de mot de passe ni de création de compte séparée) ; le nom est demandé une fois, après la première connexion ; (2) `GET /me` fournit le profil, l'entreprise active et, par entreprise, le rôle et les permissions effectives — l'app masque ce que le serveur refuserait (`payments:verify`, `payment_methods:manage`) au lieu de tester « rôle = propriétaire » ; (3) le rafraîchissement de session renvoie l'entreprise active (lue dans le jeton) ; (4) `cogs`/`profit` du tableau de bord peuvent être absents ; (5) URL de base `…/api/v1` et enveloppe d'erreur `{ error: { code, message } }`.
- **Restent à traiter (hors portage)** : thème/design system THY (L0.9 : `app_theme.dart` de thyBusiness reste en place), éclatement de `mobile/app` en `packages/` (L0.10), identifiants Android/iOS (`com.thybusiness.thy_business` hérité), sélecteur d'entreprise quand un compte en a plusieurs (l'app active la plus ancienne).
