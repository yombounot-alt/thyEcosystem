...
interface LlmProvider {
  ...
  capabilities(): { tools: boolean; vision: boolean; streaming: boolean; embeddings: boolean; maxContext: number };
...
- **Résilience** : timeouts, retries bornés, *circuit breaker*, **repli** sur un second fournisseur, dégradation gracieuse (les cartes de données déterministes restent affichées si le LLM tombe).
...
| `WRITE` | **Jamais direct.** Flux « **proposer → confirmer → exécuter** » : l'IA renvoie une *action proposée* (`ai_actions`), l'utilisateur confirme dans l'UI, puis l'exécution passe par la commande normale (avec ses gardes et son audit). Aucun outil destructeur. |
...
- Quotas via entitlements (`ai.messages.monthly`) + **plafonds de coût** globaux et par utilisateur ; alertes de dépense ; cache des résultats d'outils (TTL court) ; *prompt caching* si supporté ; **streaming SSE** pour la latence perçue.
