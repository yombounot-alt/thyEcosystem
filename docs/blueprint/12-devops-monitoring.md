...
- **Migrations** : job `migrate` **avant** le déploiement des applications ; **expand/contract** ; pas de *down* en prod (roll-forward) ; migrations lourdes en plusieurs étapes.
- **Déploiement ≠ release** : les fonctionnalités partent derrière des **feature flags** ; les *kill-switches* couvrent paiement, IA, SMS, modules.
...
|---|---|---|
| **Erreurs & crashs** | Sentry (API, worker, admin, Flutter) | Regroupement, releases, symboles, *breadcrumbs* expurgés |
