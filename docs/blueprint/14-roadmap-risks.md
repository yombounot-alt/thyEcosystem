...
**Fin de phase** : **payments v1 (abonnements uniquement)** avec le premier PSP réel *(coupe-feu : si retard, activation manuelle des abonnements pendant la bêta)*.
...
Notation 1 (faible) → 5 (très élevée). *Domaine* = richesse des règles métier ; *Technique* = difficulté d'ingénierie ; *Ext.* = dépendance à des tiers ; *Réglem./Risque* = exposition légale/fraude.
