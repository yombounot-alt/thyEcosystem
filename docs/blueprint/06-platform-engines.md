...
interface PaymentProvider {
  ...
  capabilities(): {
    collect: boolean; refund: boolean; payout: boolean;
    escrowOrSplit: boolean; recurring: boolean;
    countries: string[]; currencies: string[]; methods: PaymentMethodKind[];
  };
...
- **Push** : charge **minimale** (pas de PII/montants sensibles sur l'écran de verrouillage), *deep link* vers la route, `collapse_key` pour éviter le spam ; nettoyage des jetons invalides (`UNREGISTERED`).
